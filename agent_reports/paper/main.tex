...
$p(t^\star\mid\tau)$ over which transitions caused an episode's outcome. This
framing exposes a structural constraint---the \emph{IS-pairing requirement}---that
any non-uniform replay proposal must satisfy: the IS correction weight must be
matched to the proposal; changing the proposal without adjusting the weight
implicitly retargets the gradient to a different objective. Two consequences
follow. First, the multiplicative form $\mu\!\propto\!\mu_P\!\cdot\!w_{\text{sem}}$
is the structurally correct coupling because it keeps the standard PER IS weight
trajectory-conditionally valid; the additive mixture of concurrent work
\citep{sharony2026vlmrb} does not. Second, when the VLM generator is paired with
the \emph{training simulator itself} as a symbolic verifier, the acceptance gate
acts as an \emph{adaptive filter} whose pass-band widens monotonically as policy
precision improves---making the cold-start verifier-rejection regime transitory
by construction rather than a permanent throughput tax.
...
\citet{pignatelli2024calm}'s exogenous-FM-credit-oracle program in robot RL.
The load-bearing structural principle is the \emph{IS-pairing requirement}: a
non-uniform replay proposal $\mu$ and its IS correction weight must be paired;
changing the proposal without adjusting the weight implicitly retargets the
gradient to a different objective~\citep[Sec.~3.4]{schaul2016per}.
We choose the multiplicative form $\mu_{\text{Sem}}\!\propto\!\mu_P\!\cdot\!w_{\text{sem}}$
because it keeps the standard PER weight trajectory-conditionally well-defined
under the IS-pairing requirement; the additive mixture
$\lambda\mu_{\text{VLM}}\!+\!(1\!-\!\lambda)\mu_U$ of VLM-RB
\citep{sharony2026vlmrb} violates this requirement, retargeting the gradient
...
goal-satisfaction; we ask the VLM the more failure-directional
...
that timestep, preserving the IS-pairing established in~(1).
The scheme degenerates gracefully to PER when the VLM is
...
as a controlled under-correction (V-trace-analogous) whose bias is bounded by
Eq.~\eqref{eq:bias-bound}, deferring the strictly-correct ablation to future work.
Crucially, unlike VLM-as-reward
...
A full 9-run Phase~2 sweep (3 environments $\times$ 3 seeds $\times$
500\,k steps) confirms the mechanism integrates successfully: verified-CF
reaches a mean success rate of $0.606$ across three Fetch environments,
within $\Delta\!=\!0.016$ of VLM-CF's $0.622$, and \emph{exceeds}
VLM-CF on \texttt{FetchSlide} ($0.617$ vs.\ $0.55$)---the task where
prior baselines cluster near zero and high-fidelity relabels yield the
largest return. The cold-start verifier-rejection regime (first
$\sim$200\,k steps) is transitory: as the policy improves, the verifier's
acceptance rate climbs and the physics-consistent relabels accumulate
...
failures identifies a preferred configuration on the zero-teleport axis:
\texttt{(GPT-4o, achieved\_goal)} eliminates teleport-collapse (0/6 vs.\
Opus's 4/4) and ties for highest plausibility (0.79); the goal-progress
gap (0.83 vs.\ 0.97) reflects Opus's degenerate goal-copying rather than
genuine CF quality. A second
...
A structural point about where HER sits in the replay-correction hierarchy
is worth surfacing before the next paragraph. Vanilla HER and all three
extension families above leave the replay-\emph{sampling} distribution
unchanged: a relabeled transition is appended to the buffer, after which
the standard uniform-over-buffer sampler $\mu_U$ is invoked. The IS-pairing
question---\emph{is the gradient reweighting consistent with the proposal
that produced it?}---therefore arises only trivially under HER, with
$w_{\text{IS}}\!=\!1$ and proposal $\mu_U$. HER's degree of freedom is the
buffer's \emph{contents}; the sampler is fixed. PER and the methods we
catalogue in the next paragraph (including the concurrent VLM-RB) instead
modify $\mu$ itself, at which point the IS pairing becomes load-bearing
and the choice of pairing form (multiplicative versus additive, corrected
versus uncorrected) is no longer trivial. Our Semantic PER mechanism
(\S\ref{sec:method:sper}) exercises \emph{both} degrees of freedom: a
verified-counterfactual relabel modifies the buffer contents, and a
multiplicative $w_{\text{sem}}$ modifies the sampler in a way that keeps
the IS pairing tractable (\S\ref{sec:theory}). This is the principal
structural difference from VLM-RB, which modifies the sampler but not the
contents and whose pairing form is not IS-corrected.
...
\paragraph{Demonstration-augmented replay.}
A parallel family modifies the replay distribution from the
\emph{content} side rather than the score side: expert trajectories are
pre-loaded into the buffer, typically with an elevated priority that
decays only through standard PER updates. DDPGfD~\citep{vecerik2017ddpgfd}
and DQfD~\citep{hester2018dqfd} are the canonical examples in
continuous and discrete control respectively; DAPG~\citep{rajeswaran2017dapg}
and the demo-bootstrapped variants
of~\citet{nair2018overcomingexploration} use the same two-mechanism
recipe (seed the buffer with successes, sample them more often than
fresh online transitions) and report large sample-efficiency gains on
sparse-reward manipulation. Two mechanisms are conflated in this
recipe---\emph{what} is in the buffer and \emph{how often} it is
sampled---and the relative contribution of each is rarely ablated.
A small auxiliary sweep on MetaWorld~\citep{yu2019metaworld}
(\S\ref{sec:disc:metaworld}; full setup, results table, figures, and
code trace in Appendix~\ref{app:metaworld}) attempts to disentangle
them and surfaces a methodological subtlety: under the standard
Schaul~\citep{schaul2016per} ``insert new transitions at the running
$\max$-priority'' convention, pinning demonstrations at a fixed
elevated priority is mathematically equivalent to flat priorities,
because the $p_i^\alpha/\sum_j p_j^\alpha$ normalization is invariant
under a multiplicative constant applied uniformly across all entries.
The two variants therefore reduce to the same sampler whenever the
pinned demo priority is below or equal to the largest TD-error-induced
$\max$-priority the online stream subsequently inherits---an
implementation-detail collapse, not a methodological one, but one that
affects how the demo-priority mechanism is typically built on top of
PER infrastructure. Cleanly testing demo priority requires either
inserting online transitions at $\texttt{default\_priority}$ rather
than $\max$-priority, capping online priorities below the pinned demo
floor, or maintaining demo and online entries in two separate pools
mixed at a fixed ratio (the SACfD/DAPG approach). Semantic~PER
(\S\ref{sec:method:sper}) is not vulnerable to the same collapse:
$w_{\text{sem}}$ varies continuously across transitions within the
same episode and across episodes, so its action on the sampler is not
absorbed by the constant-scale invariance.
...
Crucially, only data-stream methods that modify $\mu$ ever face the
IS-pairing requirement (\S\ref{sec:related}, PER paragraph; \S\ref{sec:theory}):
TD-target methods leave $\mu_U$ unchanged, so the question of
\emph{which IS weight is consistent with the modified proposal} does not
arise for them. The IS-pairing question is therefore a defining structural
feature of data-stream integration---not of foundation-model use in RL
generally---and it is what makes the multiplicative-vs-additive blending
choice between our approach and VLM-RB a methodological question with a
principled answer rather than an engineering preference.
...
by the replay-sampling distribution rather than by a planner. The methodological
additions on top of CaLM's planner-only consumption are two: the
multiplicative-with-PER coupling and the simulator-verified counterfactual
gate (\S\ref{sec:method:verified}). Importantly, CaLM consumes its oracle
signal \emph{outside} the replay-sampling distribution---it modifies a
planner's reward or subgoal, not $\mu$---so it never encounters the
IS-pairing requirement. Semantic PER is the first instantiation in this
lineage that must answer the IS-pairing question (because it does modify
$\mu$), and the multiplicative-with-PER coupling is the answer. VLM-RB
\citep{sharony2026vlmrb} also modifies $\mu$ but gives a different
(additive) answer that does not preserve the IS-correction pairing
(\S\ref{sec:related}, PER paragraph). This places our contribution inside
the emerging causal-credit-assignment family---a more useful placement than
``VLM in replay'' (an engineering label) and a more accurate one than ``a
new credit-oracle category'' (CaLM precedes us on the oracle program;
we differ on the IS-pairing axis).
...
\textbf{Adaptive-filter behavior under a symbolic verifier.}
A second consequence of pairing the VLM generator with the \emph{training
simulator itself} as verifier---rather than a learned or proxy
verifier---is that the acceptance rate is a monotone function of policy
precision. Concretely: a VLM-proposed action sequence is accepted iff the
forked rollout (started from the failed episode's snapshot) crosses
$d_{\text{th}}\!=\!0.05$\,m of the goal under the unmodified sparse
reward. The probability that an arbitrarily-proposed 4-vector sequence
satisfies this predicate depends on how close the snapshot's gripper and
object configuration already are to the goal manifold; in
sparse-reward manipulation that distance is itself an inverse function
of the current policy's competence. The verifier therefore acts as an
\emph{adaptive filter} whose pass-band widens as the policy improves: at
cold-start the snapshot states are far from the goal manifold,
short-horizon corrective actions cannot cross $d_{\text{th}}$, and the
VLM-proposed candidate stream is rejected wholesale; as the policy
approaches near-success behavior, the snapshot states cluster near the
goal manifold, the same VLM proposals (now near-success completions) cross
$d_{\text{th}}$, and verified-CF transitions begin to accumulate. The
two failure modes the verifier guards against---teleport-collapse (a
prompt-architectural pathology, \S\ref{sec:method:prompt}) and
sparse-reward zero-information writes (the analysis in
\S\ref{sec:method:verified})---are both suppressed at every regime, but
their suppression mechanism shifts: at cold-start the verifier rejects
\emph{everything} (so neither failure mode contaminates the buffer), and
in the convergent regime the verifier rejects \emph{only the
miscalibrated subset} while admitting an in-distribution stream of
physics-consistent relabels. This adaptive-filter view is what makes the
cold-start verifier-rejection regime structurally \emph{transitory}
rather than a permanent throughput tax: Phase~2's
$0.617$~$\to$~\texttt{vlm\_cf}'s~$0.55$ crossover on
\texttt{FetchSlide} (\S\ref{sec:exp:inflight}) is the empirical
realization of this prediction. A learned-verifier choice
\citep{chuck2025hint,wu2025rlvrworld} can in principle attain the same
asymptotic acceptance rate, but its acceptance threshold drifts as the
learned model's own training distribution shifts under the policy's data,
breaking the monotone-in-policy-precision structure that the symbolic
verifier inherits for free.
...
\paragraph{Setup: what this section formalizes.}
Sections~\ref{sec:related}'s differentiation of Semantic PER from VLM-RB,
Section~\ref{sec:method:sper}'s headline construction
$\mu_{\text{Sem}}\!\propto\!\mu_P\!\cdot\!w_{\text{sem}}$, and
Section~\ref{sec:method:verified}'s verifier gate all rest on a single
structural principle which we state here once, formally, before the
derivation: the \emph{IS-pairing requirement}. A non-uniform replay scheme is
specified by a pair $(\mu,w_{\text{IS}})$---a proposal distribution over
buffer transitions and an importance-sampling correction weight applied to
the sampled gradient---and any change to $\mu$ must be matched by a change
to $w_{\text{IS}}$ that returns the corrected estimator to the same target
objective $\mathbb{E}_U[\ell]$. Changing $\mu$ without adjusting
$w_{\text{IS}}$ does not produce a noisier estimator of the same objective;
it produces a clean estimator of a \emph{different} objective. The rest of
this section turns this pairing requirement into a concrete derivation
(\S\ref{sec:theory}, Eq.~\ref{eq:headline}) and into a falsifiable
empirical prediction (the closing paragraph of this subsection): a
correctly-paired multiplicative VLM proposal should track PER closely when
the VLM is poorly calibrated and pull ahead when it is reliable, and
crucially \emph{cannot} exceed the privileged-information ceiling set by an
oracle counterfactual relabeler on the same buffer-shape---a prediction
borne out empirically by the HER@1M $=$ Oracle-CF@1M $= 0.583$ tie on
FetchPickAndPlace (\S\ref{sec:exp:inflight}, $\Delta = 0.000$), which bounds
the headroom for any exogenous-FM-proposal scheme on that task family.
...
\begin{equation}
  ...
  \label{eq:loss}
\end{equation}
Uniform replay $\mu_U(i)=1/|\mathcal{D}_B|$ is the canonical
unbiased-but-variance-heavy choice: the pair $(\mu_U,w_{\text{IS}}\!=\!1)$
trivially satisfies the pairing requirement because no reweighting is
needed when the proposal already equals the target distribution.
Prioritized replay~\citep{schaul2016per} is the first non-trivial example:
the proposal $\mu_P(i)\propto(|\delta_i|+\varepsilon)^\alpha$ concentrates
mass on high-error transitions, and the paired IS correction
\begin{equation}
  w_{\text{IS}}(i) \;=\; \bigl(|\mathcal{D}_B|\cdot\mu_P(i)\bigr)^{-\beta},
  \qquad \beta\colon\beta_0 \to 1.
  \label{eq:per-is}
\end{equation}
recovers (an annealed estimator of) the uniform objective: in the
$\beta\!=\!1$ limit the reweighted gradient is an unbiased self-normalized
IS estimator of $\mathbb{E}_U[\ell]$, with $\mu_P$ supplying variance
reduction~\citep[Sec.~3.4]{schaul2016per}. The proposal and the correction
move together; one cannot be modified in isolation. This is the pairing
template every subsequent construction in the section inherits, and the
choice of multiplicative versus additive proposal-shaping
(Eq.~\ref{eq:headline} versus the additive VLM-RB mixture) is the choice
of \emph{which} pairing-preserving family one writes the modification in.
...
\paragraph{Horizon caveat and baselines calibration.}
Our 500\,k-step (and 1\,M-step post-hoc) training budget is intentionally
shorter than the canonical Fetch+HER horizon of 4.75\,M timesteps reported
by \citet{plappert2018multigoal} and 8\,M timesteps reported by
\citet{andrychowicz2017her}, both of which use DDPG. At 4.75\,M, the
canonical HER+DDPG asymptotes are approximately $0.99$ / $0.95$ / $0.75$ on
\texttt{FetchPush} / \texttt{FetchPickAndPlace} / \texttt{FetchSlide}
respectively (\citealp{plappert2018multigoal}, Fig.\ 3), and
\citet{zhao2018energy} independently report a HER baseline of $0.938$ on
\texttt{FetchPickAndPlace} at 50 epochs. \citet{plappert2018multigoal} note
that ``it took around 2{,}000{,}000 timesteps to reach a median success
rate of $0.9$'' on Push---i.e., canonical training itself does not saturate
until well past our budget. Our absolute success rates at 500\,k are
therefore not directly comparable to these asymptotes; the comparisons we
draw are between \emph{methods at matched horizon}---all curves share the
same 500\,k-step budget---and the Semantic-PER vs.\ uniform PER gap (rather
than absolute SR) is the load-bearing claim. The 1\,M-step HER reference
run on \texttt{FetchPickAndPlace} has now completed: mean
$0.583$ over $n\!=\!3$ seeds ($0.35 / 0.45 / 0.95$), versus $0.167$ at
250\,k. This exactly matches the Oracle-CF@1M mean of $0.583$
($n\!=\!3$), indicating that at the convergence horizon, privileged
simulator state and ground-truth corrective actions provide
\emph{zero headroom} over vanilla HER on this task. Our
\emph{vlm\_cf}@500\,k value of $0.367$ reaches $\approx\!63\%$ of
HER@1M's asymptote at half the training budget---an efficiency claim
that survives the new asymptotic comparator.
...
\caption{\textbf{Final evaluation success rate across the three Fetch tasks
(mean $\pm$ SE, $n\!=\!3$ seeds; individual seeds overlaid as dots).}
\emph{Methods are reported at heterogeneous training horizons} --- the
italic gray stamp under each bar gives the training budget for that cell:
Uniform and PER at 3M steps; HER at 250k steps; \emph{vlm\_cf} and
\emph{verified\_cf} (ours) at 500k steps; Oracle-CF at 250k steps for
FetchPush and FetchSlide and 1M steps for FetchPickAndPlace.
\textbf{Within-horizon comparisons (same stamp value) are statistically
meaningful}; \textbf{cross-horizon comparisons should be read as efficiency
claims (more learning per training step) rather than asymptotic dominance.}
The dashed gray line in each environment marks the HER baseline at 250k for
visual reference. As a transparency note, an earlier pre-fix variant of this
figure (Uniform/PER/Semantic-PER-Oracle/Semantic-PER-GPT-4o, 500k each) used
a known-buggy all-variant prompt without the teleport gate that silently
admitted $\sim$67\% degenerate counterfactuals as relabels; the corrected
\emph{vlm\_cf} and \emph{verified\_cf} bars shown here replace those numbers
(see Sections~\ref{sec:exp:prompt}--\ref{sec:exp:real} for the bug and fix).}
\label{fig:headline}
\end{figure}
...
of Eq.~\eqref{eq:headline} go through---and, equivalently, breaking the
IS-pairing requirement of \S\ref{sec:theory} (any change to the proposal
must remain trajectory-conditional for the paired correction to be
well-defined). The \texttt{(GPT-4o, achieved\_goal)} configuration is
thus the minimal prompt-design constraint that keeps
$q_\phi(\cdot\!\mid\!\tau)$ $\tau$-dependent at all. Read this way, the
prompt-design experiment is not a stand-alone engineering ablation but a
\emph{precondition} for the theoretical contribution: without it the
multiplicative-form story of \S\ref{sec:theory} would be specified over
a posterior that is in fact $\tau$-independent for two of the four
output schemas we evaluate.

\paragraph{From a one-axis screen to a $2\!\times\!2$ structural grid.}
Table~\ref{tab:prompt-variants} screens four prompt variants
(\textsc{narrative}, \textsc{action}, \textsc{achieved\_goal},
\textsc{all}) on a single axis---\emph{which output field the prompt
asks the model to emit}---and identifies the teleport-collapse attractor
that motivates the production configuration. The mechanistic account in
the previous paragraph (\emph{prompt-length-dependent attentional
dilution} on the system-message-side, plus an in-context-goal numerical
attractor on the user-message side) factorizes naturally into two
\emph{independent} prompt-design axes that the single-axis screen
conflates: (a)~\emph{goal-visibility}---whether the prompt exposes
\texttt{desired\_goal} as a literal numeric triplet at all---and
(b)~\emph{trajectory-context}---whether numerical per-keyframe block
positions and end-effector coordinates accompany the rendered frames.
The four cells of the resulting $2\!\times\!2$ grid (\emph{knows-goal /
blind} $\times$ \emph{visual-only / visual+numeric}) decouple the
copy-attractor mechanism (axis~a) from the geometric-reasoning capacity
of the VLM (axis~b). The hypothesis stated formally in
\S\ref{sec:exp:inflight}, Part~(G), is that the \emph{blind+numeric}
cell maximizes plausibility while minimizing teleport-collapse, because
withholding the goal coordinate removes the direct copy attractor while
providing explicit trajectory geometry restores the
$\tau$-conditional information the model needs to localize a
\emph{corrective} position. Translated into the
\S\ref{sec:theory} language: the $2\!\times\!2$ grid is an empirical
test of which prompt-design axes preserve the trajectory-conditional
structure of $q_\phi(\cdot\!\mid\!\tau)$ on which Eq.~\eqref{eq:headline}
relies. Cell-level numbers are pre-registered (Part~(G)) so that the
selection of axis~a as the load-bearing one---if borne out---cannot be
read as post-hoc.

\paragraph{A second empirical test: the $p_{\text{CF}}$ sweep.}
The multiplicative-IS form of Eq.~\eqref{eq:headline} predicts that an
imperfect VLM proposal $q_\phi$ injected at a high mixing rate
$p_{\text{CF}}$ should degrade performance relative to a small-$p_{\text{CF}}$
regime in which the HER relabel still supplies most of the learning
signal: the multiplicative form treats $w_{\text{sem}}$ as a
\emph{modifier} of $\mu_P$ rather than as the proposal itself, and a
high-$p_{\text{CF}}$ injection effectively replaces $\mu_P$ with the
noisy VLM proposal in the relabeled fraction of episodes. We sweep
$p_{\text{CF}}\!\in\!\{0.00, 0.10, 0.25, 0.50, 0.75, 1.00\}$ on
\texttt{FetchPickAndPlace} (3 seeds $\times$ 500\,k steps; identical
Sonnet-4.5 \texttt{achieved\_goal} generator; W\&B tag
\texttt{path\_c\_vlm\_cf\_psweep\_2026-05-12}); see
\S\ref{sec:exp:inflight}, Part~(G), and \S\ref{sec:discussion}, item~(vii),
for the in-flight summary table.\footnote{At time of writing, 2 of 6
cells ($p_{\text{CF}}\!=\!0.00$, $p_{\text{CF}}\!=\!0.25$) are
fully complete at $n\!=\!3$; the other four cells have partial reads
($n\!<\!3$). We do not advance the partial reads as a test result; the
full 18-run sweep will be reported in the camera-ready version.}
The directional read of the partial sweep is consistent with the
prediction: the two fully-complete cells $p_{\text{CF}}\!=\!0.00$ and
$p_{\text{CF}}\!=\!0.25$ achieve essentially identical means
($0.330\!\pm\!0.045$ and $0.327\!\pm\!0.107$; $n\!=\!3$ each), while
the partial reads at $p_{\text{CF}}\!\in\!\{0.75, 1.00\}$ track
$0.04$--$0.10$. Two readings of this pattern are jointly compatible
with our theory and with each other: (i)~at the current calibration
level of $q_\phi$ on Fetch, the VLM proposal cannot dominate the HER
relabel as a learning signal, so the curve is monotonically decreasing
in $p_{\text{CF}}$ above some small-injection plateau; (ii)~the
multiplicative-IS form makes this a feature rather than a bug, because
a correctly-paired multiplicative modifier degenerates to $\mu_P$ when
$q_\phi$ is near-uniform (\S\ref{sec:theory}) and any cell that
\emph{outperforms} $p_{\text{CF}}\!=\!0.00$ does so via genuine $\tau$-conditional
signal rather than via a mixture-objective shift. The $p_{\text{CF}}\!=\!0.25$
production value is therefore not a tuned hyperparameter but a structural
prediction of the framing, and the falsifier is unambiguous: any cell at
$p_{\text{CF}}\!\ge\!0.50$ that, at full $n\!=\!3$, exceeds the
$p_{\text{CF}}\!=\!0.25$ mean by more than one SE would force a revision
of the multiplicative-form claim toward an explicit-mixture-objective
account.
...
Convergent evidence comes from the Phase~2 VLM-CF sweep
(Section~\ref{sec:exp:inflight}, part~(C)): the same prompt template---varying
only the single task-description sentence---drove the VLM credit oracle across
all three Fetch environments for 500\,k steps, successfully generating relabels
that produced the task-performance gains reported in~(C) and~(D). The
cross-environment operation required no prompt re-engineering, consistent with
the single-template transfer claim of this section.
...
\paragraph{Policy-precision dependence and the bridge to the verifier.}
Findings (i) and (iii) share a structural property worth naming
explicitly: both VLM-side error modes are
\emph{policy-precision-dependent}, and the dependence runs in
\emph{opposite directions} for the two pipelines we ship. As the policy
improves, the distribution of failures shifts from chaotic
block-off-table episodes (the synthetic regime of
Section~\ref{sec:exp:prompt}) to near-miss contact-task failures with
small $\lVert g-\mathrm{ag}_{t^\star}\rVert$. This shift
\emph{degrades} the \textsc{position}/\textsc{action}-emitting prompt
families along two axes: the achieved-goal counterfactual approaches
the desired goal in expectation (finding (i)), so $q_\phi$ drifts
toward the teleport-Dirac that
Section~\ref{sec:method:prompt} catalogs as the dominant
$q_\phi$-degeneracy regime; and the
action-axis sign vector becomes harder to read from a single keyframe
because the displacement to recover is small (finding (iii)). A
prompt-only pipeline therefore has its \emph{worst} VLM-side error
profile in precisely the policy-precision regime in which it would
otherwise have its largest marginal value---the converged-policy
near-miss regime where one well-placed counterfactual could break a
sample-efficiency plateau. The verified-CF mechanism
(Section~\ref{sec:method:verified}) inverts this dependence by
construction: its acceptance gate fires on simulator-confirmed
sparse-reward crossing rather than on VLM-axis-direction correctness,
so the same policy-precision improvement that shrinks displacement
vectors \emph{raises} the verifier's joint-event base rate toward one
(the snapshot state $\sigma$ sits closer to the goal, making the
5\,cm threshold easier to cross within $N\!=\!50$ verifier steps).
This is the conceptual reason the cold-start verifier-rejection regime
documented in
Section~\ref{sec:exp:inflight}(D) and Limitation~(ix) is transitory:
the verifier's failure mode and the prompt-only pipeline's failure
mode have \emph{anti-correlated} dependence on policy precision, so
the two mechanisms have complementary regimes of utility and the
verifier is the right tool exactly where the prompt-only pipeline
degrades. The 0/6 Sonnet teleport rate on PickAndPlace (a
mid-air task that admits a non-trivial waypoint) and the verified-CF
\texttt{FetchSlide} result---$0.617$ vs.\ \texttt{vlm\_cf}'s $0.55$
where prior baselines cluster near zero---are two reflections of the
same phenomenon under the two complementary mechanisms.
...
\textbf{(C) VLM-CF sweeps (Phase~2, attempt~5) --- all 9 runs complete.}
...
All 9 VLM-CF runs (3 envs $\times$ 3 seeds) have completed at 500\,k steps.
...
\emph{\texttt{FetchPickAndPlace}}: seeds~42/123/999 reached $0.25/0.35/0.50$
(mean~$=0.367\!\pm\!0.08$).
This exceeds HER@250\,k on the task ($0.133$) but falls below the
Oracle-CF kill threshold; the kill verdict from~(A) stands.
These runs provide ablation evidence for the verifier mechanism; VLM-CF
hindsight yields strong task performance at the 500\,k horizon on Push
and Slide in particular.
Subsection~(D) reports the parallel verified-CF runs and demonstrates that
the simulator-verification mechanism approaches the same convergence-horizon
performance.

\textbf{(D) Verified-CF convergence-horizon results --- all 9 runs complete.}
The 9 verified-CF runs (3 envs $\times$ 3 seeds $\times$ 500\,k steps) in the
Phase~2 sweep completed alongside the VLM-CF arm reported in~(C).
Results at 500\,k steps:
\emph{\texttt{FetchPush}}: seeds~42/123/999 reached $0.65/0.90/1.00$
(mean~$=0.85\!\pm\!0.14$).
\emph{\texttt{FetchSlide}}: seeds~42/123/999 reached $0.60/0.65/0.60$
(mean~$=0.617\!\pm\!0.03$).
\emph{\texttt{FetchPickAndPlace}}: seeds~42/123/999 reached $0.25/0.30/0.50$
(mean~$=0.35\!\pm\!0.10$).
Mean across three environments: $(0.85+0.617+0.35)/3 = 0.606$.
The corresponding VLM-CF mean is $(0.95+0.55+0.367)/3 = 0.622$.

The two methods are essentially tied in aggregate ($\Delta = -0.016$).
On \texttt{FetchSlide}, verified-CF exceeds VLM-CF ($0.617$ vs.\ $0.55$),
the task on which prior baselines cluster near zero and where high-fidelity
relabels are most valuable.
On \texttt{FetchPickAndPlace}, the two methods tie within noise ($0.35$ vs.\ $0.367$).
On \texttt{FetchPush}, verified-CF trails marginally ($0.85$ vs.\ $0.95$),
consistent with the task being near-saturated at this horizon.

At convergence, the simulator-verified counterfactual mechanism approaches
and on \texttt{FetchSlide} exceeds the vanilla vlm\_cf result.
The cold-start verifier-rejection regime documented in Limitation~(ix) is
transitory: as the policy improves, the verifier's acceptance rate rises
and the small set of verified counterfactuals provides a low-variance,
physics-consistent training signal.

\textbf{(E) Sharony VLM-RB baseline --- implemented, pending launch.}
...
Modal once the Phase~2 capacity is released; head-to-head
...
\textbf{(F) Convergence-horizon comparator: Oracle-CF@1M ties HER@1M on PickAndPlace.}
As a separate, post-hoc experiment not part of the pre-registered kill test,
we ran Oracle-CF \emph{and} a matched HER comparator on
\texttt{FetchPickAndPlace} to 1\,M steps with $n\!=\!3$ seeds each.
\textbf{Oracle-CF@1M}: three seeds reached
final eval success rates of $0.30 / 0.90 / 0.55$ (mean $0.583$).
\textbf{HER@1M}: seeds~42/123/999 reached $0.35 / 0.45 / 0.95$
(mean $0.583$). The two means are \emph{identical at two decimal places}:
$\Delta\!=\!0.000$, well inside the $\pm 0.10$ pre-registered kill
threshold. \textbf{Interpretation.} At the convergence horizon for
\texttt{FetchPickAndPlace}, privileged simulator state plus a
ground-truth corrective action provides \emph{zero headroom} over vanilla
HER's achieved-future relabel. This is the strongest possible version of
the kill verdict from (A): the negative result is not an artifact of the
250\,k decision horizon. It strengthens---rather than retracts---the
pre-registered conclusion that the credit-assignment headroom available
to any VLM-in-replay method on Fetch is bounded above by HER itself on
PickAndPlace. For \emph{vlm\_cf}@500\,k ($0.367$), the implied efficiency
claim sharpens: 63\% of HER's 1\,M asymptote at half the budget.

\textbf{(G) Prompt-design ablation matrix --- pre-registered, in flight.}
...
A small auxiliary sweep---$3$ tasks $\times$ $4$ replay variants
$\times$ $5$ seeds $=$ $60$ SAC runs at $500$\,k env steps, $130.5$
L4-GPU-hours---on MetaWorld~v3~\citep{yu2019metaworld} corroborates
this paper's central premise that the replay sampler is load-bearing
in sparse-reward continuous control. Full setup, per-variant
results table with bootstrap CIs, figures, the implementation-detail
code trace, and the pre-registered follow-on appear in
Appendix~\ref{app:metaworld}
(Table~\ref{tab:mw:results}, Figures~\ref{fig:mw:final}--\ref{fig:mw:curves}). Reward was replaced by
$\texttt{float(success)}$ on \texttt{drawer-open-v3},
\texttt{sweep-into-v3}, and \texttt{button-press-v3}, holding all
other SAC hyperparameters fixed. Vanilla TD-error PER cut
env-steps-to-$0.8$-success by roughly half over uniform replay (median
across reaching seeds: uniform $455$k\,/\,$220$k\,/\,$80$k;
PER $375$k\,/\,$300$k\,/\,$213$k) and lifted final success at
$500$k by $+0.44$, $+0.12$, $+0.40$ on the three tasks respectively.
We had pre-registered the opposite hypothesis---that TD-error PER
contributes little in sparse-reward SAC because near-uniform TD errors
dominate the early training phase---and the data refute it. The
mechanism is the same one that motivates Semantic~PER's
priority-shaping in this paper: once the first reward-bearing
transitions occur, their elevated TD errors push the running
$\texttt{max\_priority}$ that all subsequent online inserts inherit
under Schaul's convention, propagating the signal forward as a
recency boost layered on top of the TD-error bias. The independent
corroboration is useful because the MetaWorld task family does not
admit HER (no goal-conditioned reward function), so the result
isolates the priority mechanism from the relabeling mechanism that
dominates Fetch.
Demonstration content (10 scripted-expert episodes pre-loaded
into the buffer) saturates the two easier tasks---$1.00 \pm 0.00$
final success across all five seeds on \texttt{drawer-open-v3} and
\texttt{button-press-v3}, with steps-to-$0.8$ falling to $176$k and
$32$k---but is insufficient on the harder \texttt{sweep-into-v3}
($0.32 \pm 0.39$ final, despite all five seeds first crossing $0.8$
success around $192$k steps). The exploration-then-stability split is
the regime where mere buffer seeding underprovides credit-assignment
signal: demos suffice to first stumble onto a successful trajectory
but the policy does not stay there. This is precisely the regime
Semantic~PER and verified-counterfactual hindsight were designed for,
and a head-to-head Semantic~PER evaluation on MetaWorld manipulation
is pre-registered as a follow-on. The auxiliary sweep also confirms
the demo-replay~$\equiv$~demo-priority equivalence under the Schaul
max-priority insert convention discussed in \S\ref{sec:related}
(``Demonstration-augmented replay''): every (task, seed) pair
produced bit-identical eval curves between the two variants, isolating
the collapse to the priority-pinning implementation rather than to the
demonstration-augmentation idea itself, and supporting the
methodological recommendation made there.
...
A parallel sweep over the counterfactual mixing probability $p_{\text{CF}}\!\in\!\{0.0, 0.10, 0.25, 0.50, 0.75, 1.00\}$ --- the per-episode probability of activating a VLM-CF relabel versus falling back to the standard HER relabel --- is running at time of writing (18 runs, FetchPickAndPlace-v4, 500\,k steps, W\&B tag \texttt{path\_c\_vlm\_cf\_psweep\_2026-05-12}).
Partial results (6/18 complete) are directionally consistent with the IS-posterior framing's prediction: $p_{\text{CF}}\!=\!0.00$ (pure HER) and $p_{\text{CF}}\!=\!0.25$ achieve comparable means ($0.330 \pm 0.045$ and $0.327 \pm 0.107$, $n\!=\!3$ seeds each), while the three highest-mass cells ($p_{\text{CF}}\!\in\!\{0.75, 1.00\}$, partial reads) are tracking $0.04$--$0.10$---consistent with a noisy $q_\phi$ at high CF injection rate overriding the HER relabel signal that provides most of the learning leverage on Fetch.
We pre-register $p_{\text{CF}}\!=\!0.25$ as the production value (already used in Phase~2) pending the full 18-run result.
...
(ix) \emph{Transitory cold-start verifier-rejection regime.} The simulator-as-verifier
...
steps. During cold-start---approximately the first 200\,k steps, when the
snapshot state $\sigma$ sits far from the goal because the policy has not yet
learned to approach the object---this joint event has very low base rate.
Our Phase~2 FetchPickAndPlace seed~42 hit a 100\% verifier-rejection rate
over the first $\approx$80 VLM calls (steps 1.4k--2.8k of 100k), producing
a regime where verified-CF is operationally indistinguishable from a SAC+HER
run with extra VLM-API overhead.
However, our Phase~2 final results (Section~\ref{sec:exp:inflight}, part~(D))
show this regime is transitory: as the policy improves, the verifier's
acceptance rate climbs and verified-CF reaches a mean success rate of
$0.606$ across three environments at 500\,k steps---within $0.02$ of
vlm\_cf's $0.622$, and exceeding it on \texttt{FetchSlide} ($0.617$
vs.\ $0.55$).
The verifier acts as a noise filter that pays off once the policy clears a
minimum performance threshold.
Practical mitigations for the cold-start phase---a base-policy warm-up
before enabling the verifier, a longer horizon $N$, or a soft-acceptance
relaxation (e.g., $r\!\ge\!-\rho_r$ for some $\rho_r\!<\!\eta$)---are
pre-registered for a follow-on study but not run here.
...
channel to a strictly costlier HER.
This cold-start regime is transitory: Phase~2 results show verified-CF
reaching mean $0.606$ across three environments at 500\,k steps, within
$0.02$ of vlm\_cf's $0.622$ and exceeding it on \texttt{FetchSlide}
(Limitation~(ix)).
