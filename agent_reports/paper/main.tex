...
SAC+HER+verified-CF against SAC+HER+\emph{oracle}-CF, where the oracle
returns the ground-truth corrective action by solving inverse kinematics
from the failure state to the desired goal---a perfect-information ceiling
that the VLM cannot reach in zero-shot.
This comparison serves three diagnostic purposes simultaneously.
\textbf{First}, if oracle-CF matches or exceeds verified-CF, the gap is
signal-quality-bounded: the VLM's action proposals are the performance
bottleneck, not the simulator-fork mechanism.
\textbf{Second}, if oracle-CF and HER tie (the pessimistic scenario
anticipated by the pre-registered Phase~1 decision rule in
Section~\ref{sec:exp:inflight}), the null result is informative: it
says that even a perfect corrective action from the failure state does not
meaningfully accelerate SAC convergence at 250\,k steps from cold start,
likely because the SAC actor needs many more such relabels to update its
policy in the right direction.
\textbf{Third}, the oracle-CF run used a corrected midpoint anchor
(block-position-to-goal rather than end-effector-to-goal after an
earlier pipeline bug; see Section~\ref{sec:discussion}); the rerun
provides the first valid oracle-CF measurement on FetchPush.
The smoke harness (\texttt{scripts/n1\_smoke\_verified\_cf.py}) supports
the oracle swap as a one-flag change; final numbers are awaited from the
in-flight runs.
