...
\textbf{(C) VLM-CF and verified-CF sweeps (Phase~2) --- in flight.}
...
blocked (API failures fall back to \texttt{achieved\_goal} relabels). These
runs will provide ablation evidence for the verifier mechanism; based on the
pre-registered kill verdict for Oracle-CF, we do not expect them to exceed
the HER-Fetch ceiling identified in~(A).
