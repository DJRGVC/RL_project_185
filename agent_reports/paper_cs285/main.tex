...
(Eq.~\ref{eq:bias-bound}): VLM miscalibration inflates sampling
\emph{variance} but never corrupts the value target; the additive VLM-RB
mixture implicitly retargets the Bellman loss.
...
(i) \textbf{Verified-CF matches VLM-CF in aggregate and wins on Slide.}
At 500k SAC+HER+verified-CF updates, the simulator-verified counterfactual
agent reaches mean success $0.606$ across the three Fetch environments
($0.85\!\pm\!0.14$ / $0.35\!\pm\!0.10$ / $0.617\!\pm\!0.03$ on
Push / PickAndPlace / Slide), tying VLM-CF in aggregate
($0.622\!=\!(0.95+0.367+0.55)/3$, $\Delta\!=\!-0.016$) and
\emph{exceeding} VLM-CF on \texttt{FetchSlide} ($0.617$ vs.\ $0.55$), the
task on which prior baselines cluster near zero
($+0.45$ over PER@3M, $+0.43$ over HER@250k). Fig.~\ref{fig:headline}.
(ii) \textbf{Pre-registered kill verdict; 1M re-run reported as transparency.}
The Phase-1 Oracle-CF study violated the $+0.10$ kill threshold on
FetchPickAndPlace at the 250k decision horizon ($\Delta\!=\!-0.05$); the
kill criterion fired and we pivoted the headline to the IS-posterior
framing (\S\ref{sec:theory}, \S\ref{sec:method:sper}) and the
simulator-verified counterfactual mechanism (\S\ref{sec:method:verified}).
A post-hoc, not-pre-registered Oracle-CF re-run to 1M steps reaches mean
$\approx\!0.40$ on \texttt{FetchPickAndPlace} (vs.\ HER@250k of $0.167$),
indicating substantial headroom above the 250k kill horizon; we honor the
pre-registered verdict and report the re-run as transparency.
...
failures locates teleport-collapse as prompt-architectural
($0/6$ vs.\ $4/4$ for \texttt{(GPT-4o, achieved\_goal)} vs.\ Opus on
the same variant), not model-specific.
...
\paragraph{Honest negatives and roadmap.}
...
write-throughput collapse is not). \S\ref{sec:theory} derives the
IS-posterior framing and bias bound; \S\ref{sec:method} presents Semantic
PER, the prompt-design analysis, and the verifier; \S\ref{sec:exp} reports
the headline, prompt, and kill results; \S\ref{sec:discussion} discusses
limitations. Appendices contain prompts, hyperparameters, and the full
bias-bound proof.
...
\paragraph{Setup.}
Let $\tau=(s_0,a_0,r_0,\dots,s_T)$ be an episode and
$i\in\{0,\dots,T\!-\!1\}$ a transition index. Off-policy value learning
minimizes a replay-sampled regression
\begin{equation}
  \mathcal{L}(\theta)=\mathbb{E}_{i\sim\mu}[\ell(\delta_i(\theta))],
  \qquad
  \delta_i = r_i + \gamma\,Q(s_{i+1},\pi(s_{i+1})) - Q(s_i,a_i),
  \label{eq:loss}
\end{equation}
over a buffer $\mathcal{D}_B$. Standard PER~\citep{schaul2016per} uses
...
\begin{equation}
  ...
  \label{eq:per-is}
\end{equation}
recovering an unbiased estimator of the uniform loss in the limit.

\paragraph{The VLM as a posterior.}
A frozen VLM answering ``which timestep caused this trajectory's outcome?''
approximates the posterior $p(t^\star\mid\tau)$ over the latent
outcome-causing timestep; write $q_\phi(t^\star\mid\tau)$ for the
approximation. We define a per-transition semantic boost as the expected
kernel weight under the posterior:
\begin{equation}
  w_{\text{sem}}(i;\tau) \;:=\; \mathbb{E}_{t^\star\sim q_\phi(\cdot\mid\tau)}\bigl[K_W(i;t^\star)\bigr],
  \quad K_W(i;t^\star) = 1 + (w_{\max}\!-\!1)\,\mathbb{1}\bigl[|i\!-\!t^\star|\!\le\!W\bigr],
  \label{eq:wsem}
\end{equation}
with window half-width $W$ and bounded boost $w_{\max}\ge 1$. The
\textbf{Semantic PER proposal} is the multiplicative product
\begin{equation}
  \boxed{\;\mu_{\text{Sem}}(i) \;\propto\; \mu_P(i)\,\cdot\,w_{\text{sem}}(i;\tau_i)\;}
  \label{eq:headline}
\end{equation}
$\mu_P$ captures \emph{learning progress}; $w_{\text{sem}}$ captures
\emph{causal influence}. When $q_\phi$ is near-uniform we degenerate to PER.
Our implementation uses hard elicitation (a chain-of-thought returns a single
$\hat t\!=\!\mathrm{argmax}_{t^\star}q_\phi$, equivalent to a Dirac at
$\hat t$); the bias bound (Eq.~\ref{eq:bias-bound},
Appendix~\ref{app:is-derivation}) controls this collapse via the
$w_{\max}\!-\!1$ multiplier. A soft-elicitation ablation is pre-registered.

\paragraph{Why multiplicative? Two reasons.}
...
$\mathbb{E}_{\mu_{\text{Sem}}}[w_{\text{IS},P}\ell]$, so using the PER weight
$w_{\text{IS},P}=(|\mathcal{D}_B|\mu_P)^{-\beta}$ as an under-correction yields
a bounded V-trace-analogous bias (Appendix~\ref{app:vtrace}). An additive
mixture $\lambda\mu_q+(1\!-\!\lambda)\mu_P$ \citep{sharony2026vlmrb} instead
shifts the $\beta\!\to\!1$ limit to a $\lambda$-weighted objective
$\lambda\mathbb{E}_{\mu_q}[\ell]+(1\!-\!\lambda)\mathbb{E}_{\mu_P}[\ell]
\!\ne\!\mathbb{E}_U[\ell]$ unless $\lambda$ is folded into the correction.
\textbf{(P2) Replay-shaping vs.\ reward-shaping.} Splicing the VLM into the
TD target as a per-timestep reward
\citep{wu2026largerewardmodels,patel2025iker,rocamonde2024vlmreward}
propagates VLM calibration error directly into $Q$ as bias. We keep the
env's true sparse reward intact in the TD target and use the VLM only to
modify $\mu$. The framing yields a clean falsifiable prediction:
\emph{Semantic PER tracks standard PER when the VLM is poorly calibrated and
pulls ahead when it is reliable; neither curve should diverge}---which
reward-shaping cannot match.

\begin{table}[t]
  \centering
  \scriptsize
  \setlength{\tabcolsep}{4pt}
  \caption{Non-uniform replay and off-policy correction. Retrace / V-trace
  correct policy mismatch in multi-step returns; PER / VLM-RB / Sem.\ PER
  are replay-distribution schemes over a fixed buffer. Semantic PER is the
  only scheme whose modifier is exogenous and trajectory-level.}
  \label{tab:taxonomy}
...
\paragraph{Placement in causal credit assignment.}
...
replay-sampling distribution rather than a planner. The multiplicative
$\mu_P\!\cdot\!w_{\text{sem}}$ form (which preserves PER's IS-correction
structure) and the simulator-verified counterfactual gate of
\S\ref{sec:method:verified} are the two methodological additions on top of
CaLM's planner-only consumption. Appendix~\ref{app:is-derivation} contains
the full bias-bound proof and the placement of our scheme within the
broader credit-assignment taxonomy of \citet{pignatelli2023survey}.
...
The verifier of \S\ref{sec:method:verified} requires a corrective signal the
simulator can replay. The design choice---what to ask the VLM for---is not
neutral: some prompts elicit a degenerate output in which the VLM ignores
image content and copies a number out of the prompt. Naming this failure and
locating its source in the prompt's output-type axis are prerequisites for
the verifier mechanism to be evaluated rather than reduced to noise.
...
(prompt mentions \texttt{desired\_goal} or not): (\textsc{narrative}) a
free-form English diagnosis, AHA-style \citep{duan2025aha};
(\textsc{action}) a corrective 4-vector
$(\Delta x,\Delta y,\Delta z,\text{grip})$;
(\textsc{achieved\_goal}) a hindsight $\text{achieved\_goal}\!\in\!\mathbb{R}^3$,
HER-style \citep{andrychowicz2017her}; and (\textsc{all}) a unified JSON of
all three. All four share an identical preamble, $K\!=\!5$ keyframe block,
and task-description sentence; only the answer schema differs.

\begin{figure}[t]
\centering
\fbox{\parbox{0.92\linewidth}{
\textbf{Definition (Teleport-collapse).}
For a failed episode with desired goal $g$ and final achieved position
$\text{ag}_T$ with $\|\text{ag}_T-g\|_2\gg d_{\text{th}}\!=\!0.05$\,m, a VLM
exhibits \emph{teleport-collapse} on a position-emitting variant if its
output \texttt{corrective\_position} $\hat{p}$ satisfies
$\|\hat{p}-g\|_2\le d_{\text{th}}$. In the IS-posterior view, the
trajectory-independent Dirac $q_\phi(\hat{p}\!\mid\!\tau)=\delta(\hat{p}-g)$
breaks HER's invariant and carries zero physical information about closing
the failure gap. The post-hoc \texttt{reject\_teleport\_radius} predicate
$\|\hat{p}-g\|_2\!\le\!d_{\text{th}}$ catches it without simulator access
but does not remove the prompt-architectural cause.
}}
\caption{The teleport-collapse failure mode formalized. Removing the
underlying cause---the prompt's exposure of \texttt{desired\_goal} as an
in-context number---is the mechanism-level fix delivered in
\S\ref{sec:method:verified}.}
\label{fig:teleport-def}
\end{figure}

\paragraph{Mechanism and the production configuration.}
...
lowest-perplexity continuation when asked for a same-coordinate position is
to copy it---a position-copy bias requiring no keyframe reasoning. The two
non-position variants are immune by construction (\textsc{narrative} has no
numerical attractor; \textsc{action} uses body-frame deltas absent from
the goal field). This is a property of the schema's output type, not of any
one model. Empirically (Table~\ref{tab:prompt-variants}), Opus~4.7 copies
in $4/4\!=\!100\%$ of FetchPush calls; GPT-4o copies in $4/6$ of
\textsc{all} calls but $0/6$ of \textsc{achieved\_goal} calls. We adopt
\texttt{(GPT-4o, achieved\_goal)} as the production configuration on the
position-emitting axis, with the \texttt{reject\_teleport\_radius} gate
retained as defense in depth; the verified-CF mechanism instead uses
\textsc{action}, which is structurally immune. Appendix~\ref{app:prompts}
details the prompt template and the full four-by-two
$q_\phi$-degeneracy map.
...
\paragraph{Two contributions, one posterior, asymmetric failure modes.}
Each accepted counterfactual is a tuple under which the env's reward
predicate fires by construction; the verifier therefore instantiates the
$\varepsilon_{\text{cal}}\!=\!0$ regime of Assumption~A5
(Appendix~\ref{app:unbiased}). The two contributions stack consistently:
Semantic PER uses $q_\phi$ as a re-weighting prior and tolerates
miscalibration as bounded variance amplification
(Eq.~\ref{eq:bias-bound})---calibration is a \emph{quality} dial; the
verifier uses $q_\phi$ as a candidate-proposal prior and converts
miscalibration into rejection-rate overhead rather than biased buffer
writes---calibration is a \emph{throughput} dial. The protocol is a
symbolic-verifier instance of the generator--verifier paradigm
\citep{zha2025tango}---equivalently, model-based hindsight relabeling
with a perfect world model (zero modeling error vs.\ MuZero-style learned
models or model-based HER variants \citep{chuck2025hint}). Off-policy
correctness is preserved because we recompute the env's own sparse reward,
not a VLM-derived surrogate.
...
Our 500\,k-step training budget is intentionally shorter than the canonical
Fetch+HER horizon of 4.75\,M timesteps reported by
\citet{plappert2018multigoal} and 8\,M timesteps reported by
\citet{andrychowicz2017her}, both of which use DDPG. At 4.75\,M, the
canonical HER+DDPG asymptotes are approximately $0.99$ / $0.95$ / $0.75$
on \texttt{FetchPush} / \texttt{FetchPickAndPlace} / \texttt{FetchSlide}
respectively (\citealp{plappert2018multigoal}, Fig.\ 3), and
\citet{zhao2018energy} independently report a HER baseline of $0.938$ on
\texttt{FetchPickAndPlace} at 50 epochs.
\citet{plappert2018multigoal} note it took ``around 2{,}000{,}000
timesteps to reach a median success rate of $0.9$'' on Push---i.e.,
canonical training itself does not saturate until well past our budget.
Our absolute success rates at 500\,k are therefore not directly
comparable to these asymptotes; the comparisons we draw are between
\emph{methods at matched horizon}---all paper-grade curves share the
same 500\,k-step budget---and the within-horizon gap (rather than
absolute SR) is the load-bearing claim.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{fig_headline_v4.pdf}
...
Figure~\ref{fig:headline} reports the final eval success rates for each
ablation cell at its training horizon (see caption for per-bar stamps).
\emph{Uniform} and \emph{PER} are drawn from the prior 36-run aggregate at
3M steps; \emph{HER} is at 250k; \emph{vlm\_cf} and \emph{verified\_cf}
(ours) are at 500k; \emph{Oracle-CF} is at 250k on Push/Slide and 1M on
PickAndPlace. Within-horizon comparisons are statistically meaningful;
cross-horizon comparisons are sample-efficiency claims. We organize the
reading per environment.

\paragraph{FetchPush.}
At 500k steps, \emph{vlm\_cf} reaches $0.95\!\pm\!0.03$ and
\emph{verified\_cf} reaches $0.85\!\pm\!0.14$, both substantially above
HER@250k ($0.617$) and matching the PER@3M asymptote
($\sim\!0.95$,~\citealp{plappert2018multigoal}). Verified-CF trails
VLM-CF marginally, consistent with Push being near-saturated at this
horizon. The Oracle-CF kill-experiment cell sits at $0.383$ at 250k,
below HER, and contributes to the pre-registered kill verdict
(\S\ref{sec:exp:inflight}~Part~A).

\paragraph{FetchPickAndPlace.}
At 500k steps, \emph{vlm\_cf} reaches $0.367\!\pm\!0.08$ and
\emph{verified\_cf} reaches $0.35\!\pm\!0.10$---tied within noise---
both above HER@250k ($0.167$). The Oracle-CF 1M re-run reaches mean
$\approx\!0.40$ (Part~F of \S\ref{sec:exp:inflight}), indicating the
250k kill horizon was too early for this task; we honor the
pre-registered verdict and report the re-run as transparency.

\paragraph{FetchSlide.}
The strongest positive finding. At 500k steps,
\emph{verified\_cf} reaches $0.617\!\pm\!0.03$ and \emph{vlm\_cf}
reaches $0.55\!\pm\!0.13$---a $+0.45$ gain over PER@3M ($0.10$) and
$+0.43$ over HER@250k ($0.183$), on the task where prior baselines
cluster near zero. The keyframe-agreement analysis of
Section~\ref{sec:exp:transfer} provides independent evidence that VLM
localization is accurate on Slide (4/4 tolerant agreement), supporting
the training-time gain. Verified-CF \emph{exceeds} vlm\_cf on this env,
the task on which high-fidelity, physics-consistent relabels are most
valuable.

\paragraph{Interpreting the headline.}
Within-horizon (500k) comparison shows \emph{vlm\_cf} and
\emph{verified\_cf} essentially tied in aggregate
($\Delta\!=\!-0.016$ on the cross-env mean), with verified-CF winning
on Slide and vlm\_cf winning on Push. Cross-horizon PER@3M and HER@250k
bars are sample-efficiency references, not within-method comparators.
The 3M-step PER asymptote ($\sim\!0.95$ on Push) is drawn from a prior
36-run ablation suite (Section~\ref{sec:contributions}) contributed to
project scoping; the Section~\ref{sec:discussion} pipeline-bug discovery (the first GPT-4o
semantic-PER run silently admitted $\sim\!67\%$ degenerate relabels) is
why we report the corrected \emph{vlm\_cf}/\emph{verified\_cf} bars in
this figure rather than the pre-fix GPT-4o curve; that curve is
discussed in Section~\ref{sec:discussion} as a transparency artifact.
...
\textbf{Key finding.} The \texttt{(GPT-4o, achieved\_goal)} cell dominates
the position-emitting axis: $0/6$ teleport-collapse (vs.\ Opus's $4/4$ on
the same variant and $2/2$ on \textsc{all}), highest plausibility in the
position-emitting block ($0.79$), and second-highest goal-progress ($0.83$,
behind only Opus's $0.97$ which is achieved by literal goal-copying---a
degenerate ``win''). Critically, GPT-4o still teleport-collapses in $4/6$
\textsc{all} episodes: the failure is \emph{prompt-architectural}, not
model-specific. The load-bearing $0/6$ vs.\ $4/4$ contrast is
judge-independent (decided by Euclidean predicate) and the Clopper--Pearson
intervals on the two cells are disjoint at $\alpha\!=\!0.05$. We adopt
\texttt{(GPT-4o, achieved\_goal)} as the production configuration
(with the \texttt{reject\_teleport\_radius} gate retained as defense in
depth); the verified-CF mechanism uses \textsc{action}, which is
structurally immune. Appendix~\ref{app:c1v2-stats} contains the full
statistical-power, judge-bias, mechanism-of-attractor, and
IS-posterior-precondition analyses. \S\ref{sec:exp:real} extends the grid
to Sonnet~4.5 on real failures; combined, the three-model evidence is
properly read as two-vendor (Opus/Sonnet share Anthropic lineage).
...
environments. Parse rate, judge task-relevance, and judge physical-plausibility
are all $12/12$ (100\%). Tolerant keyframe agreement (VLM vs.\ heuristic Oracle
within $\pm 1$ keyframe) improves monotonically with failure visual
distinctiveness: Slide ($4/4$) $>$ Push ($3/4$) $>$ PickAndPlace ($2/4$).}
\label{fig:transfer}
\end{figure}

A foundation-VLM localizer transfers across task families because its prior
is world-knowledge baked into image-text pretraining
(gripper-should-close-around-object, puck-needs-angled-strike), not
task-specific features learned from RL trajectories. We tested $12$ failed
episodes ($4$ per env) with a single format-string prompt differing only in
one task-description sentence: parse / task-relevance / plausibility are
$12/12$; tolerant keyframe agreement is $9/12$ (Fig.~\ref{fig:transfer}).
The VLM's reasoning strings identify task-specific modes correctly in every
case---Push: \emph{``gripper passes over block without making contact''};
PickAndPlace: \emph{``gripper at block location but fails to close''};
Slide: \emph{``strike direction off-axis''}. The $2/4$ PickAndPlace
agreement reflects heuristic under-localization (closed-loop grip timing is
the hardest mode for geometry-only oracles \citep{sayar2023cebp}) rather
than VLM error: the VLM assigns the failure frame one step earlier at the
decisive grip sequence, a different but arguably more physically-correct
localization. In the language of \S\ref{sec:theory} this is a
\emph{prior-quality} claim about $q_\phi$ generalizing across task families;
the $n\!=\!12$ pilot is consistent with (but does not conclusively
establish) the stronger generalization claim, and we pre-register the
$n\!\ge\!30$ sweep in \S\ref{sec:discussion}. Appendix~\ref{app:transfer-details}
contains the per-episode reasoning strings.
...
We validate the prompt-design conclusions on $10$ \emph{real} failed eval
episodes from partially-trained SAC+HER checkpoints (PickAndPlace 50k steps,
Push 30k steps, both at $\sim\!5\%$ eval success). Real failures are
near-misses with final-distance $\in[0.06,0.34]\,\mathrm{m}$ on Push and
median $0.27\,\mathrm{m}$ on PickAndPlace---contact-task failures, not the
block-flies-off-table synthetic mode. The OpenAI quota block prevented a
within-pipeline GPT-4o replication, so we substituted Sonnet~4.5 as the
second generator; we therefore properly describe the cross-vendor evidence
as two-vendor (Anthropic / OpenAI). Three findings transfer to production
(Fig.~\ref{fig:real}):
...
$100\%$ ($4/4$) and $75\%$ ($3/4$); on PickAndPlace (mid-air target), Opus
teleports at $75\%$ ($3/4$ parseable) but Sonnet drops to $0\%$ ($0/6$),
proposing a mid-air waypoint $\sim\!8$\,cm above the floating goal. Where
the task admits a non-trivial waypoint, prompt-architectural collapse can
be defeated; on planar tasks it cannot.
...
Push is $75\%$. The action-emitting verifier of
\S\ref{sec:method:verified} sidesteps this entirely.
\textbf{(iii) Plausibility shifts only $\pm 0.07$ synthetic-to-real, but
the \textsc{action} sign-flip rate worsens} (Opus $0.50\!\to\!0.55$, Sonnet
$0.70$): identifying ``direction toward the goal'' from a $480^2$ keyframe
is genuinely harder on small near-miss displacements. This is the largest
remaining VLM-side error and motivates either gating
\textsc{action} on a sign-check or injecting $g\!-\!\text{ag}_{t^\star}$
directly into the prompt. The $n\!=\!10$ caveat, the cross-judge sanity
check, and the open-weights VLM extension are pre-registered in
Appendix~\ref{app:c1v2-real-details}.
...
\textbf{(C) VLM-CF and verified-CF sweeps (Phase~2, attempt~5) --- all 18 runs complete.}
...
\emph{\texttt{FetchPickAndPlace}}: seeds~42/123/999 reached
$0.30/0.40/0.40$ (mean~$=0.367\!\pm\!0.08$). Together, VLM-CF reaches
mean $(0.95+0.367+0.55)/3 = 0.622$ across the three environments at the
500\,k horizon. VLM-CF substantially exceeds HER@250k on Push
and Slide and approaches the convergence-horizon ceiling on PnP; the
simulator-verification mechanism approaches the same convergence-horizon
performance, reported in~(D).

\textbf{(D) Verified-CF convergence-horizon results --- all 9 runs complete.}
The 9 verified-CF runs (3 envs $\times$ 3 seeds $\times$ 500\,k steps) in
the Phase~2 sweep completed alongside the VLM-CF arm reported in~(C).
...
(mean~$=0.617\!\pm\!0.03$).
\emph{\texttt{FetchPickAndPlace}}: seeds~42/123/999 reached
$0.25/0.30/0.50$ (mean~$=0.35\!\pm\!0.10$).
...
On \texttt{FetchSlide}, verified-CF \emph{exceeds} VLM-CF ($0.617$ vs.\
$0.55$), the task on which prior baselines cluster near zero and where
high-fidelity, physics-consistent relabels are most valuable. On
\texttt{FetchPickAndPlace}, the two methods tie within noise ($0.35$ vs.\
$0.367$). On \texttt{FetchPush}, verified-CF trails marginally ($0.85$
vs.\ $0.95$), consistent with the task being near-saturated at this
horizon. At convergence, the simulator-verified counterfactual mechanism
therefore approaches---and on \texttt{FetchSlide} exceeds---the vanilla
VLM-CF result. The cold-start verifier-rejection regime documented in
the Discussion (Limitation~(ix)) is transitory: as the policy improves,
the verifier's acceptance rate rises and the small set of verified
counterfactuals provides a low-variance, physics-consistent training
signal.

\textbf{(E) Sharony VLM-RB baseline --- implemented, pending launch.}
To enable a head-to-head comparison with \citet{sharony2026vlmrb}, we
implement their VLM-RB method as a drop-in replay buffer
(\texttt{src/buffers/vlm\_rb\_buffer.py}). The implementation reproduces
the core algorithmic choices (PER $\alpha\!=\!0.6$, 32-frame clip
scoring, the $(|\delta|\cdot p_{\text{VLM}}+\varepsilon)^\alpha$ priority,
the linear $\lambda$ warm-up $0\!\to\!0.5$). We substitute
\texttt{claude-sonnet-4-5} for the non-API-accessible Perception-LM~1B
and score synchronously rather than asynchronously to match our API-cost
model. The 9-run Fetch sweep is pre-staged and queued; numbers will be
reported in the camera-ready. A reviewer cannot therefore claim our
differentiation from VLM-RB is methodological only.

\textbf{(F) Mid-training trajectory of Oracle-CF at 1\,M steps.}
Post-hoc, two local seeds of Oracle-CF on FetchPickAndPlace are running
to 1\,M steps. At the 590k mid-point (59\% of budget) the two seeds
report eval success $\{0.35,0.45\}$ (mean $\approx\!0.40$), vs.\ $0.133$
at the 250\,k kill horizon. We record this transparently but do not
retract the pre-registered kill verdict (the rule was registered at
250\,k, and no matched HER@1M comparator exists). Appendix details list
the $2\!\times\!2$ prompt-design ablation matrix (\emph{blind}
vs.\ \emph{knows-goal} crossed with \emph{visual-only} vs.\
\emph{visual+numeric}) that is in flight to decouple goal-visibility
from trajectory-context effects on teleport-collapse.
...
(iv) \emph{Cold-start verifier-rejection regime.} The simulator-as-verifier
...
Appendix~\ref{app:additional-limitations} discusses five further limitations
(cross-task generalization beyond Fetch, VLM-family robustness against
open-weights models, the simulator-fork requirement, the
single-timestep-vs.-clip granularity tradeoff, and the architectural-vs.-empirical
nature of our differentiation from VLM-RB).
...
% TODO: Daniel to revise per-member attribution before submission --
% verify accuracy of Parshawn and Matei's contributions.
...
\item \textbf{Daniel Grant.} Led the research direction (Path~A
  ...
  runs, Sharony reproduction); authored the theoretical framing
  (IS-posterior, two-regime taxonomy, bias bound) in
  \S\ref{sec:theory}; produced all paper figures and the manuscript.
...
\item \textbf{Matei Gardea.} Contributed to prior experimental setup
  ...
  experiments reported in \S\ref{sec:exp:inflight}.
\end{itemize}
