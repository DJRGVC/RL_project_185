...
\paragraph{Problem.}
Sparse-reward goal-conditioned manipulation is fundamentally a temporal
credit-assignment problem. In Gymnasium-Robotics Fetch
environments---Push, PickAndPlace, Slide---a randomly-initialized policy
succeeds in $<\!1\%$ of episodes, and the lone terminal reward bit at the end
of a 50-step trajectory decays exponentially through TD-bootstrapping, leaving
the bulk of every episode effectively invisible to the optimizer.
Hindsight Experience Replay (HER) addresses this by relabeling failed
trajectories with achieved goals, but provides no signal about \emph{which}
timestep along the trajectory caused failure, and standard Prioritized
Experience Replay (PER) cannot recover this signal in sparse-reward regimes
because the TD-error itself is the bottleneck. Concurrent work
(VLM-RB,~\citet{sharony2026vlmrb}) uses frozen vision-language models to
inject a learned-prior \emph{additive} priority bias; we argue that the
\emph{multiplicative} form is the principled choice and pair it with a
guarantee-providing simulator-verified counterfactual channel.

\paragraph{Methodological contributions.}
We make two contributions. \textbf{(1) Semantic PER as importance-sampled
posterior reweighting:} we recast VLM-guided experience replay as a
trajectory-conditional posterior $q_\phi(t^\star\!\mid\!\tau)$ over the
failure-causing timestep, and show that multiplying the standard TD-error
priority by $q_\phi$ admits a clean IS-correction analysis with an explicit
bias bound (Eq.~\ref{eq:bias-bound}): VLM miscalibration inflates sampling
\emph{variance} but never corrupts the value target. The additive VLM-RB
mixture, by contrast, implicitly retargets the Bellman loss.
...
sequence} rather than a hindsight goal, then executing it in a simulator
fork and only admitting physics-consistent, reward-positive relabels into
the replay buffer (confidence~1.0, zero modeling error).

\paragraph{Headline findings.}
(i) \textbf{vlm\_cf headline.} At 500k SAC+HER+VLM-CF updates on FetchPush
seed~0 the verified-CF agent reaches 0.95 success; on FetchSlide it reaches
0.55, surpassing the SAC+HER baseline on both tasks
(Fig.~\ref{fig:headline}). (ii) \textbf{Pre-registered Phase~1 kill verdict
+ 1M update.} Our Phase~1 oracle-heuristic study triggered the pre-registered
kill criterion (no separation by 300k updates); we honor the kill and
re-allocate compute to Phase~2 (verified-CF), which crosses the criterion at
1M updates. (iii) \textbf{Prompt-design and cross-task transfer.} A
two-vendor three-model sweep (GPT-4o / Opus~4.7 / Sonnet~4.5) on 6 synthetic
and 10 real Fetch failures identifies the teleport-collapse failure as
prompt-architectural, not model-specific; a single prompt template yields
12/12 cross-task annotations across Push / PickAndPlace / Slide.
(iv) \textbf{Sharony reproduction.} A faithful re-implementation of the
concurrent VLM-RB priority formula (same $\lambda$-schedule, clip length,
substituting Sonnet~4.5 for the non-API-accessible PerceptionLM) is
pre-staged in our codebase for head-to-head comparison.

\paragraph{Honest negative result.}
We document a \emph{cold-start regime} in which the verified-CF channel
collapses to a strictly costlier HER: when the snapshot state $\sigma$ sits
far from the goal because the underlying policy has not yet learned to
approach the object, the joint event (VLM proposes a corrective action;
the sequence reaches the goal in $\le N$ verifier steps) has near-zero
base rate. Our FetchPickAndPlace verified-CF run hit a 100\%
verifier-rejection rate over the first $\sim$80 VLM calls. The two failure
modes are asymmetric: Semantic-PER degeneracy inflates variance (bounded);
Verified-CF degeneracy extinguishes write-throughput entirely.

\paragraph{What this report contains.}
The remainder of this report describes the method
(\S\ref{sec:method}), an IS-posterior theory section
(\S\ref{sec:theory}, body proof sketch + bias bound), the full experiment
suite (\S\ref{sec:exp}, headline + prompt-design + cross-task +
real-data + kill-experiment + Sharony reproduction), and a
discussion of nine concrete limitations (\S\ref{sec:discussion}).
Appendices contain the prompt templates, the Oracle~v3 heuristic
localizer, full hyperparameter tables, and the bias-bound proof.
