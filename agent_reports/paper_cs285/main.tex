...
(i) \textbf{VLM-CF on Push and Slide.} At 500k SAC+HER+VLM-CF updates,
FetchPush reaches $0.95\!\pm\!0.03$ success ($+0.33$ over HER@250k) and
FetchSlide reaches $0.55\!\pm\!0.13$ ($+0.37$ over HER@250k, $+0.45$ over
PER@3M); Fig.~\ref{fig:headline}.
(ii) \textbf{Pre-registered kill verdict.} The Phase-1 Oracle-CF study
violated the $+0.10$ kill threshold on FetchPickAndPlace
($\Delta\!=\!-0.05$), bounding the headroom for any VLM-in-replay scheme
on Fetch at 250k steps; we honor the kill and route compute to Phase~2.
...
The verified-CF channel exhibits a \emph{cold-start regime} (FetchPickAndPlace
seed~42: 100\% verifier-rejection over the first $\sim\!80$ VLM calls), in
which it reduces to a strictly costlier HER. The Semantic-PER and verified-CF
failure modes are asymmetric---miscalibration inflates Semantic-PER variance
(bounded) but extinguishes verified-CF write-throughput entirely.

\paragraph{Roadmap.}
\S\ref{sec:theory} derives the IS-posterior framing and bias bound;
\S\ref{sec:method} presents Semantic PER, the prompt-design analysis, and
the verifier mechanism; \S\ref{sec:exp} reports the headline, prompt, and
kill results; \S\ref{sec:discussion} discusses limitations.
Appendices contain prompts, the Oracle~v3 localizer, hyperparameter tables,
and the full bias-bound proof.
...
$\mathcal{L}(\theta)=\mathbb{E}_{i\sim\mu}[\ell(\delta_i(\theta))]$ over a
buffer $\mathcal{D}_B$, with TD error
$\delta_i = r_i + \gamma\,Q(s_{i+1},\pi(s_{i+1})) - Q(s_i,a_i)$. Standard
PER~\citep{schaul2016per} uses $\mu_P(i)\propto(|\delta_i|+\varepsilon)^\alpha$
and reweights each gradient by the annealed IS correction
$w_{\text{IS}}(i)=(|\mathcal{D}_B|\mu_P(i))^{-\beta}$,
$\beta\colon\beta_0\!\to\!1$, recovering an unbiased estimator of the uniform
loss in the limit.
...
methodological comparison in Table~\ref{tab:differentiation},
...
% placeholder attributions below reflect a best-guess split based on
% the session work log; please correct any inaccuracies.

\begin{itemize}\itemsep0pt
\item \textbf{Daniel Grant.} Led research direction (Path~A / Path~C
  dual-track); implemented and ran all training experiments
  (\texttt{vlm\_cf}, \texttt{oracle\_cf}, \texttt{verified\_cf},
  Phase~1 kill, Sharony reproduction); authored the theoretical
  framing (IS-posterior derivation, bias bound, two-regime taxonomy
  in \S\ref{sec:theory}); produced all paper figures and wrote the
  manuscript.
\item \textbf{Parshawn Gerafian.} Contributed to initial project
  scoping and the prior 36-run ablation suite (PER / Semantic-PER
  with GPT-4o / Oracle-heuristic) that is referenced as foundational
  data in \S\ref{sec:exp:headline}.
\item \textbf{Matei Gardea.} Contributed to prior pipeline development
  and the ablation infrastructure that underlies the Phase~1 /
  Phase~2 experiments reported in \S\ref{sec:exp:inflight}.
\end{itemize}
